...
public class Game {
	...
	public void assemblePieces() {
		...
		Armored a1 = new Armored(player1, this, "Tirion Fordring");
		board[p1InitRow + 1][ind1.remove(0)].setPiece(a1);
		Armored a2 = new Armored(player2, this, "The Lich King");
		board[p2InitRow][ind2.remove(0)].setPiece(a2);

		Medic m1 = new Medic(player1, this, "Anduin Wrynn");
		board[p1InitRow + 1][ind1.remove(0)].setPiece(m1);

		Medic m2 = new Medic(player2, this, "Kel'Thuzad");
		board[p2InitRow][ind2.remove(0)].setPiece(m2);

		Ranged r1 = new Ranged(player1, this, "Rexxar");
		board[p1InitRow + 1][ind1.remove(0)].setPiece(r1);

		Ranged r2 = new Ranged(player2, this, "Sylvanas Windrunner");
		board[p2InitRow][ind2.remove(0)].setPiece(r2);

		Speedster sp1 = new Speedster(player1, this, "Maiev Shadowsong");
		board[p1InitRow + 1][ind1.remove(0)].setPiece(sp1);

		Speedster sp2 = new Speedster(player2, this, "Illidan Stormrage");
		board[p2InitRow][ind2.remove(0)].setPiece(sp2);

		Super su1 = new Super(player1, this, "Alexstraza");
		board[p1InitRow + 1][ind1.remove(0)].setPiece(su1);

		Super su2 = new Super(player2, this, "Deathwing");
		board[p2InitRow][ind2.remove(0)].setPiece(su2);

		Tech t1 = new Tech(player1, this, "Archmage Antonidas");
		board[p1InitRow + 1][ind1.remove(0)].setPiece(t1);

		Tech t2 = new Tech(player2, this, "Gul'dan");
		...
		for (int j = 0; j < boardWidth; j++) {
			board[p1InitRow][j].setPiece(new SideKickP1(this, "Water Elemental"));
			board[p2InitRow + 1][j].setPiece(new SideKickP2(this, "Infernal"));
		}
		for (int i = 0; i < board.length; i++)
